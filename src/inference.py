...
def parse_arguments():
    ...
    parser.add_argument("-nhl", "--num_layers", type=int, default=6)
    parser.add_argument("-sz", "--hidden_size", nargs="+", type=int, default=[128, 128, 128, 128, 128, 128])
...
def get_model_path(args):
    ...
    while True:
        ...
        if os.path.exists(numbered_path): i += 1
        else: break
    if i > 1: return save_path.replace(".npy", f"_{i-1}.npy")
...
def evaluate_model(model, X_test, y_test):
    ...
    cm = confusion_matrix(y_true, preds, labels=np.arange(10))
